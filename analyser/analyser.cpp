...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseConstantDeclaration() {
		...
		while (true) {
			...
			// <常量声明语句>
			next = nextToken();
			if (!next.has_value() || next.value().GetType() != TokenType::IDENTIFIER) //类型必须是标识符
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNeedIdentifier);
			if (isDeclared(next.value().GetValueString())) //不可以重复定义
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrDuplicateDeclaration);
			addConstant(next.value()); //添加常量
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
		...
		while(true)
		{
			auto next = nextToken(); //预读
			if(!next.has_value()) //没有值
				return {};
			if(next.value().GetType() != TokenType::VAR) { //有值但是不是VAR
				unreadToken();
				return {};
			}
			...
			if(isDeclared(next.value().GetValueString())) //不可重复定义
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrDuplicateDeclaration);
			...
			if(!next.has_value() || next.value().GetType() != TokenType::EQUAL_SIGN) {
				//判断";"
				addUninitializedVariable(tmpVar.value());
				...
				addVariable(tmpVar.value());
				auto err = analyseExpression(); //变量表达式
				if (err.has_value())
					return err;
				next = nextToken(); //判断';'
				if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON) {
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
		...
		miniplc0::Token tmpVar = next.value();
		...
		if (isUninitializedVariable(tmpVar.GetValueString()))
			updateUninitialized(tmpVar);
		//生成存储指令
		_instructions.emplace_back(Operation::STO, getIndex(tmpVar.GetValueString()));
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseFactor() {
		...
		switch (next.value().GetType()) {
			...
			case IDENTIFIER: {
				...
				_instructions.emplace_back(Operation::LOD, getIndex(tmp));
				break;
			}
			...
			case LEFT_BRACKET: {
				...
				if(!next1.has_value() || next1.value().GetType() != TokenType::RIGHT_BRACKET)
					return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrIncompleteExpression);
				break;
			}
			default:
	...
	void Analyser::updateUninitialized(const Token& tk) {
		int32_t pos = _uninitialized_vars[tk.GetValueString()];
		_vars[tk.GetValueString()] = pos;
